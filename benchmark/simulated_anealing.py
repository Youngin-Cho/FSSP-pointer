...
class PBS(Annealer):
    ...
    def __init__(self, state):
        super(PBS, self).__init__(state)  # important!
    ...
    def energy(self):
        block_num = len(self.state)
        process_num = len(self.state[0])

        blocks = np.array(self.state)
...
if __name__ == '__main__':
    ...
    # pbs = PBS(init_state, data_all)
    #
    # pbs.set_schedule(schedule)
    # pbs.copy_strategy = "slice"
    # state, e = pbs.anneal()
    # print()
    # # print('parameters: ', schedule)
    ...
    for j in filename:
        ...
        for i in range(30):
            ...
            init_state = list_data

            pbs.append(PBS(list_data))

            # schedule = pbs.auto(minutes=0.3)
            pbs[i].set_schedule(schedule)

            pbs[i].copy_strategy = "slice"
            state, e = pbs[i].anneal()
        ...
        res_list.append(sum(result) / len(result))
